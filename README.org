...
Most of the interface between the model (which was made using another python library, Tensorflow) and the page was handled in a single [[file:frontend/untexifyweb/app/views.py][views.py]] file. Python acted as the glue between Django and Tensorflow, which was extremely helpful and satisfying to work with.
...
:END:
#+EXPORT_FILE_NAME: /home/shortcut/git/untexify/frontend/untexifyweb/app/templates/app/home.html
https://github.com/rgri/untexify
...
<html lang="en-us">
  <head>
    ...
    <title>Canvas</title>
    <script src="{% static 'app/script.js' %}" defer></script>
  </head>
  <body>
